...
function setWebhook($url) {
    try {
        file_get_contents(API_URL . 'setWebhook?url=' . urlencode($url));
        return true;
    } catch (Exception $e) {
...
if ($update) {
    ...
    // This is a normal web request (not from Telegram)
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Set webhook when accessing the root URL with ?set_webhook parameter
        if (isset($_GET['set_webhook'])) {
            $webhook_url = "https://" . $_SERVER['HTTP_HOST'] . "/";
            if (setWebhook($webhook_url)) {
                echo "Webhook set successfully to: " . $webhook_url;
            } else {
                echo "Failed to set webhook";
            }
        } else {
            echo "Bot is running. Access with ?set_webhook to configure.";
        }
    }
}
